...
nocolor () {
	cat $1 |
		sed 's/\x1b\[[0-9;]*m//g' |		# SGR (color & effects) ANSI
		sed 's/\x1b\[[0-9]\?[HJ]//g' | 	# clear ANSI
		sed 's/\x1b][0-9]\+;.*\x07//g' |	# clear OSC
		sed 's/\x0d//g'			# remove carriage return
	}
...
# assemble nasm source and output hex
# TODO: add switch for architecture
scode() {
	obj=$(mktemp)
	raw=$(mktemp)
	nasm -felf64 $1 -o $obj
	ld --oformat=binary -o $raw $obj
	hex=$(xxd -p $raw | tr -d '\n')
	echo $hex
...
# assemble and run nasm source w/ strace
scode-trace() {
	obj=$(mktemp)
	bin=$(mktemp)
	nasm -felf64 $1 -o $obj
...
function start_tmux() {
	...
	if type tmux &>/dev/null; then
		# make sure we're not already in a tmux session
		if [[ -z "$TMUX" ]]; then
